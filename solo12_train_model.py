...
class Solo12Env(gym.Env):
    def __init__(self):
        ...
        # Observations: robot's position, orientation, velocities, and joint states
        obs_high = np.array([np.inf] * 37, dtype=np.float32)  # Updated to match the actual observation size
    ...
    def get_obs(self):
        ...
        obs = np.concatenate([position, orientation, linear_velocity, angular_velocity, joint_positions, joint_velocities]).astype(np.float32)
        return obs
    ...
    def _compute_reward(self, obs):
        # Reward can be defined based on various factors such as forward velocity, stability, etc.
        # Here, we reward forward motion along the x-axis
        position = obs[:3]
